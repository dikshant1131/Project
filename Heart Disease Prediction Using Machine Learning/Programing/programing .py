...
from MYAOL import Save_Data_MySql
...
def analysis():
    ...
    try:
        ...
        messagebox.showerror('missing0','Please select gender!!')
        return
...
##lets check all are worning or not :
    print('A is age: ',A)
    print('B is  gender :',B)
    print('C is  cp:',C)
    ...
    # reshape hte numpy array as we are predicting for anly on instance
...
#info:
    ...
    Label(Icon_window,text='chol - serum cholostoral in mg/d1',font='arial 11').place(x=20,y=220)
    Label(Icon_window,text='fbs - fasting blood sugar > 120 mg/d1(1 = true; 0 = false)',font='arial 11').place(x=20,y=250)
    Label(Icon_window,text='restecg - resting electrocardiographic results (0 = false)',font='arial 11').place(x=20,y=280)
    ...
    Label(Icon_window,text='oldpeak - ST depreddion induced by exercise relative to rest ',font='arial 11').place(x=20,y=370)
    Label(Icon_window,text=' slope - the slope of the peak exercise ST segment (0 = upsloping; 1 = flat; 2 = downloping)',font='arial 11').place(x=20,y=400)
    Label(Icon_window,text='ca - number of major vessrls (0-3) colored by  flourosopy',font='arial 11').place(x=20,y=430)
...
#############it is used to cloass tha window #################################################################################################################################################################
def Logout():
    root.destroy()
...
Label(text='Dikshant',font='arial 13',bg='#df2d4d',fg='black').place(x=10,y=10)
...
###################radio button###############3
Label(Detail_entry,text='sex:',font='arial 13').place(x=10,y=10)
Label(Detail_entry,text='fbs:',font='arial 13').place(x=180,y=10)
Label(Detail_entry,text='exang:',font='arial 13').place(x=335,y=10)
...
cp_combobox=Combobox(Detail_entry,values=['0 = typical angina','1 = atypical angina','2 = non-anginal pain','3 = asymptomic'],font='arial 12',state='r',width=11)
...
repport_background=Label(image=square_report_image,bg=background)
repport_background.place(x=1120,y=320)

report=Label(root,font='arial 25 bold',bg='white',fg='#8dc63f')
report.place(x=1170,y=550)

report1=Label(root,font='arial 25 bold',bg='white')
...
Label(image=graph_image).place(x=600,y=480)
Label(image=graph_image).place(x=860,y=480)
...
def changemode():
    ...
    if button_mode:
        choice='non_smoking'
        mode.config(image=non_shoking_icon,activebackground='white')
        button_mode=False
    else:
        choice='smoking'
